...
### 8. Client VM Setup
After configuring the Domain Controller and network services, the next step was to create and prepare a client machine to join the domain environment.

- Created a new **Windows 10 Client** virtual machine in **Oracle VirtualBox**.  
- Configured the VM with:
  - **1 network adapter** set to **Internal Network** (same network as the Domain Controller).  
  - Allocated appropriate **RAM and storage** for testing domain connectivity.  
- Verified network settings to ensure the client and Domain Controller are on the same internal subnet.

#### 📸 Visual Reference
Below is a screenshot showing the **Client1 virtual machine** configured with the **Internal Network** adapter, ready to join the `OMARDOMAIN.com` domain.

<p align="center">
  <img src="client1.jpg" alt="Windows 10 Client VM Configuration" width="600"/>
</p>

> *Screenshot:* The VirtualBox **Network settings** window displaying **Client1** connected to the internal network.

### 9. Logging in with Generated User Account
After creating the `Client1` VM and connecting it to the internal network, the next step was to test domain authentication by logging in with one of the bulk-generated user accounts from Active Directory.

- Powered on **Client1** and reached the login screen.  
- Selected **Other User** to sign in using a domain account.  
- Entered credentials for a user created by the PowerShell script (e.g., **A-Oryan**) from the `_EMPLOYEES` OU.  
- Successfully logged into the Windows 10 client with domain credentials, verifying proper Active Directory integration and network connectivity.

#### 📸 Visual Reference
Below is a screenshot showing the **Windows 10 login screen** for **Client1**, logging in with a domain account generated by the PowerShell script.

<p align="center">
  <img src="cript.jpg" alt="Client1 Logging in with Domain User" width="600"/>
</p>

> *Screenshot:* The Windows 10 **login screen** for `Client1` showing a domain user (`A-Oryan`) being used to log into the system.
